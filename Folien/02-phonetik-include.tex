...
Wie viele Laute haben die folgenden Wörter?
		...
		\begin{columns}
			...
			\column{.35\textwidth}
				\begin{enumerate}
					...
					\item<2> \textipa{[ l a N ]}
					\item<2> \textipa{[ b @ P a \textscr\  b \texttoptiebar{aI} t U N ]}
					\item<2>[] \textipa{[ b @ P a  b \texttoptiebar{aI} t U N ]}
				\end{enumerate} 
			\column{.15\textwidth}
				\begin{enumerate}
					\item<2>[] 4
		...
		\end{columns}
                ...
                \bigskip
       \begin{itemize}
       	\item[]<2> \textipa{\texttoptiebar{aI}} kann man als einen oder als zwei Laute zählen.
       \end{itemize}         
...
\begin{tabular}{l|p{6cm}|p{3cm}}
	...
	\hline
\textipa{\textprimstress} & Hauptbetonung 				&[\textipa{Papo\textprimstress te:k@}]\\
...
\begin{table}
	\scalebox{.9}{
		\begin{tabular}{llp{9cm}}
			1. Busch               & \only<2->{[\textipa{bUS}]}                             & \only<3->{b: bilabialer, stimmhafter Plosiv \textipa{S}:~postalveolarer, stimmloser Frikativ}                                                                                \\
			\only<4->{2. malen}    & \only<5->{[\textipa{\textprimstress ma:l@n}]}          & \only<6->{m:~bilabialer, stimmhafter Nasal; n:~alveolarer, stimmhafter Nasal, l:~alveolaer, stimmhafter Lateral}                                                             \\
			\only<7->{3. Maus}     & \only<8->{[\textipa{m\t{aU}s}]}                        & \only<9->{m:~s.o.; s:~stimmloser, alveolarer Frikativ}                                                                                                                       \\
			\only<10->{4. Achtung} & \only<11->{[\textipa{\textprimstress PaXtU\ng}]}       & \only<12->{\textipa{P}:~glottaler, stimmloser Plosiv; \textipa{X}:~velarer, stimmloser Frikativ; t:~alveolarer, stimmloser Plosiv \textipa{\ng}: velarer, stimmhafter Nasal} \\
			\only<13->{5. Genie}   & \only<14->{[\textipa{Ze:\textprimstress ni:}]}         & \only<15->{\textipa{Z}: postalveolarer, stimmhafter Frikativ, n:~s.o.}                                                                                                       \\
			\only<16->{6. zirpen}  & \only<17->{[\textipa{\t{ts}IKp@n}]}                    & \only<18->{\textipa{\t{ts}}: alveolare, stimmlose Affrikate; \textipa{K}:~uvularer, stimmhafter Frikativ; p:~bilabialer, stimmloser Plosiv; n: s.o.}                         \\
			\only<19->{7. wichtig} & \only<20->{[\textipa{\textprimstress vI\c{c}tI\c{c}}]} & \only<21->{\textipa{v}: labiodentaler, stimmhafter Frikativ; \textipa{\c{c}}: palataler, stimmloser Frikativ t: s.o.}                                                        \\
			\only<22->{8. Wald}    & \only<23->{[\textipa{valt}]}                           & \only<24->{v, l, t: s.o.}
		\end{tabular}}
\end{table}
%\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause\pause
...
\begin{frame}{Vokalviereck}
	...
	\begin{itemize}
		\item Für eine bessere Darstellung wurden die Vokale (von Daniel Jones 1920) in das sog. Vokalviereck angeordnet, welches eine \textbf{stilisierte Version des Vokalraums} darstellt.
	\end{itemize}
...
\frametitle{Triphthong (Dreilaut)}
	...
	\eal
	\ex \textipa{\t{aI5}} \ras Eier
	\ex \textipa{\t{OI5}} \ras Steuer
	\ex \textipa{\texttoptiebar{aU5}} \ras Bauer
	\zl
...
\frametitle{Übungen: Transkription}
	...
	\begin{itemize}
		...
		\column{.40\textwidth}
				\begin{enumerate}
					\item<2> \textipa{[bE͡5k.St\texttoptiebar{aI}.g5]}
					\item<2> \textipa{[kvo:.t@n.no:.t@]}
					\item<2> \textipa{[fi:l.fa\.x@s]}
					\item<2> \textipa{[pEk.\c{c}@n.Pan.na:.m@]}
					\item<2> \textipa{[b@.PEn.d@n]}
					\item<2> \textipa{[fE͡5.\textscr \texttoptiebar{aI}.z@n]}
					\item<2> \textipa{[fE͡5.P\texttoptiebar{aI}.z@n]}
					\item<2> \textipa{[P\texttoptiebar{aI}n.\texttoptiebar{ts}a:.lU N]}
					\item<2> \textipa{[ge:.@n]}
