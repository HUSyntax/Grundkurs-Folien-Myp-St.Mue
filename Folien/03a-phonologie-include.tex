...
\begin{itemize}
	...
	\item[]
	\item Wenn das Deutsche ein neues Wort wie \ab{Code} \textipa{[k@Ud]} entlehnen würde, würde dieses Wort früher oder später \gqq{eingedeutscht} werden.
...
\textbf{kanonische Aussprache} ($\approx$ Standardaussprache)\\
und die vielen möglichen \textbf{umgangssprachlichen Formen}:
%	\item[]
...
\begin{itemize}
	...
	\eal
		...
		(vgl. \ab{Lehrerin} \textipa{[le:.{\textscr}@.\alertred{\textscr}In]})
	\zl
...
\eal
\ex \textipa{[b]}, \textipa{[z]}, \textipa{[a]}, \textipa{[g]}, \textipa{[v]}, \textipa{[p]}, \textipa{[u]}
...
\item[3.] Geben Sie die \textbf{phonologische Repräsentation} der folgenden Wörter und \textbf{verschiedene phonetische Realisierungen} (\zB im Paradigma) an. Erläutern Sie anschließend den \textbf{Unterschied} zwischen den verschiedenen möglichen phonetischen Realisierungen.
...
\begin{itemize}
	...
	\begin{minipage}{0.4\textwidth}
		...
		(\only<2->{\alertred{G}}) Stimmritze (glottal)\\
		(\only<3->{\alertred{F}}) Kehlkopf (laryngal)\\
		(\only<4->{\alertred{B}}) Zahndamm (alveolar)\\
		(\only<5->{\alertred{A}}) Nasenraum (nasal)\\
		(\only<6->{\alertred{H}}) harter Gaumen (palatal)\\
		(\only<7->{\alertred{D}}) Zähne (dental)\\
		(\only<8->{\alertred{J}}) weicher Gaumen (velar)\\
		(\only<9->{\alertred{I}}) Zungenrücken (dorsal)\\
		(\only<10->{\alertred{K}}) Halszäpfchen (uvular)\\
		(\only<11->{\alertred{C}}) Lippen (labial)\\
		(\only<12->{\alertred{E}}) Zungenspitze (apikal)
	\end{minipage}
...
\settowidth\jamwidth{XXXXXXXXXXXXXXXXXXXXXXXt}
	\begin{xlist}
		...
		\ex \textipa{[t]}, \textipa{[s]}, \textipa{[n]}, \textipa{[\c{c}]}, \textipa{[l]}, \textipa{[d]}, \textipa{[r]} \loesung{3}{\textipa{[ç]} (nicht alveolar, sondern palatal)}
		\ex \textipa{[f]}, \textipa{[s]}, \textipa{[x]}, \textipa{[h]}, \textipa{[r]}, \textipa{[z]}
...
\begin{exe}
	...
	\begin{xlist}
		\ex Dieb
		\loesung{2}{\textipa{/di:b/}: \textipa{[di:p], [di:.bə]} -- Auslautverhärtung des \textipa{/b/} in der Koda}
		\ex König
		\loesung{3}{\textipa{/k\o :nıg/}: \textipa{[k\super h\o:.nıç], [k\super h\o:.nı.gə]} --} \loesung{3}{Spirantisierung des \textipa{/g/} in der Koda}
		\ex eng
