...
\item[1.] Geben Sie eine \textbf{phonetische Tranksription} der folgenden Wörter nach der \gqq{Standardaussprache} an, zeichnen Sie dabei die \textbf{Silbenstruktur} nach dem \textbf{Konstituentenmodell} und mit der \textbf{Skelettschicht}, und geben Sie die \textbf{Sonoritätsprofile} an.
...
%%%%%%%%%%%%%
%@MyP: Soll hier wirklich 2x hintereinander die Aufgabe gezeigt werden (1x unter Überschrift H direkt danach unter Lösung)???????????????????
%%%%%%%%%%%%%%%
...
\pause
\begin{xlist}
\ex \label{ex:HA2a}\textipa{[Pu.\;RI.nStInkt]} (Onsetmaximierung) \pause
\ex \label{ex:HA2b}\textipa{[Pu.\;RIn.(\,S\,)tInkt]} (Sonoritätsprinzip, \textipa{S} ist extrasilbisch) \pause
