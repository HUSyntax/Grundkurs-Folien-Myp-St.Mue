...
% 	\pause            
	...
	\scalebox{.65}{
		...
		[\alert{XP}	[BP [Adjunkt,roof]]
			[XP	[YP [Argument,roof]]
				[\alert{\MyPxbar{X}} 	[WP [Adjunkt,roof]]
...
% 	\pause            
	...
	\scalebox{.75}{
		...
		[VP 
			[\MyPxbar{V} 
				[\alert{NP}
					[\alert{AP} [\alert{erfrischenden},roof]]
					[\alert{NP}
						[\alert{\MyPxbar{N}}
...
%	%
	...
	\scalebox{.8}{
		...
		[IP [SpecIP [Subjekt Position,roof]],tikz={\node [draw,red,fit=()] {};} 
					[\MyPxbar{I} [VP [Objekte+Verb,roof]][\zerobar{I} [Flexion]],tikz={\node [draw,red,fit=()] {};} 
		]]
		\end{forest}
		}
		%\caption{NP}
...
node[anchor=east,align=center]{\textsc{dat}\\ \textsc{ziel}};}
		    [\MyPxbar{V}	[\alert{DP} [den Wagen,roof]]{\draw[<-,red] (.south west)--++(0em,-1ex)--++(-6em,0pt)
...
\begin{figure}[b]
	...
	\begin{figure}					
		...
		\caption{Struktur der CP (Haftka 1995)}
		%\label{Zeichen1}
	\end{figure}
	\end{minipage}
	%
...
%	\end{minipage}
	...
	\begin{figure}
		...
		\caption{Struktur der DP (Lenerz 1993)}
		%\label{Zeichen2}
	\end{figure}
	\end{minipage}                        
\end{figure}
				...
				[TP [ich]
					[\MyPxbar{T}
						[VP [\MyPxbar{V} 
								[DP [dir,roof]]								
								[\xxbar{V}
										[DP [ein großes Stück Kuchen,roof]]		
										[\zerobar{V} [t$_{i}$]]]]]
						[\zerobar{T} [t$_{i}$]]
					]
				]
		...
		[CP
			...
			[\MyPxbar{C} [\zerobar{C} [habe$_{i}$]]
				[TP [t$_{k}$]
					[\MyPxbar{T}
						[VP	[AdVP [heute,roof]]
							[VP [PP	[im Auto,roof]]{\draw[->,red] (.north west)--++(-7em,0em)--++(0em,14em)
...
\item Konstituenten falsch bestimmt (z.B. Tea = V?) 
