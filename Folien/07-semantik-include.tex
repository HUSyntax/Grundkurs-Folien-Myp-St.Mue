...
>>>>>>> c5153acdd02f6e9bfac4067dcbed064970e01c8c
\end{minipage}
%%
%%
\begin{minipage}{0.48\textwidth}
...
%
%	\ea Genau dann wenn ich Durst habe, trinke ich eine Cola.
%	\ex Es ist nicht der Fall, dass ich Durst habe, und es ist nicht der Fall, dass ich eine Cola trinke -- oder -- es ist der Fall, dass ich Durst habe und es ist der Fall, dass ich eine Cola trinke.
%	\z 
%	
...
\begin{block}{Inkompatibilität (Kontrarität)}
$p$ und $q$ sind miteinander inkompatibel (bzw. zueinander konträr), wenn $p$ und $q$ Widersprüche erzeugen (vgl.\ Kohyponymie).

\textit{p} und \textit{q} sind also imkompatibel (konträr), wenn \textbf{beide gleichzeitig nicht wahr} sein können, aber beide könnten gleichzeitig falsch sein.
\end{block}
...
\eal
	...
	\ex X ist eine gerade Zahl (in der Menge der natürlichen Zahlen). -- X ist eine ungerade Zahl (in der Menge der natürlichen Zahlen).
...
\hfill \alertred{Meronymie (genauer: Feder ist ein Meronym zu Vogel)}
...
\ea 
	\ea Auf dem Tisch liegt eine Rose.\\
	Auf dem Tisch liegt eine Blume.\\
	\ras \alertred{Implikation}
	
	\ex Alle Vögel können fliegen.\\
	Kein Vogel kann nicht fliegen.\\
	\alertred{\ras Paraphrase (synonyme Sätze)}
...
\begin{table}
	...
	\scalebox{.9}{\begin{tabular}{c|c|c|c|c|c}
		\textbf{p}& \textbf{q} & \textbf{p $\lor$ q} & \textbf{q $\lor$ p} & \textbf{(p $\lor$ q)} $\leftrightarrow$ \textbf{(q $\lor$ p)} & \textbf{$\lnot$ ((p $\lor$ q) $\leftrightarrow$ (q $\lor$ p))} \\ 
		\hline 
