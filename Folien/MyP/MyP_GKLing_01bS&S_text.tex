...
\begin{itemize}
	\item Folgende Module werden angenommen (vgl. \citet{Repp&Co12a}):
...
\begin{itemize}
	\item Eintrag: $\langle$\textsc{geb(en)}$\rangle$

	\begin{enumerate}
		\item[]
		\item Phonologische Information: \textipa{/ge\textlengthmark{}b\textschwa{}n/}
		\item[]
		\item Morphologische Information: {[}{[}$\langle$geb$\rangle$] + {[}$\langle$en$\rangle$]]
		\item[]
...
\begin{frame}{Phonologische Komponente}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item[]
			\item<2->[$\rightarrow$] Wieso spricht man $\langle Hund \rangle$ mit \textipa{[t]} aber $\langle Hunde \rangle$ mit \textipa{[d]} aus?
			\item[]
			\item<3->[$\rightarrow$] Kann ein Wort im Deutschen mit der Lautfolge \textipa{[Ng]} beginnen?
			\item[]
			\item<4->[$\rightarrow$] Was ist der Unterschied zwischen $\langle$\textit{HAUStürgriff}$\rangle$ und $\langle$\textit{HausTÜRgriff}$\rangle$?
		\end{itemize}		  
...
\begin{itemize}
	...
	\begin{itemize}
		\item[]
		\item<2->[$\rightarrow$] Wie hängen $\langle kaufen \rangle$ und $\langle kaufbar \rangle$ zusammen?
		\item[]
		\item<3->[$\rightarrow$] Was zeigt $\langle$\textit{-st}$\rangle$ bei der Bildung neuer Verbformen an?
	\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		\item[]
		\item[$\rightarrow$] Warum ist die eine Struktur des Wortes $\langle Bedeutungsableitung \rangle$ intuitiv nicht korrekt und die andere schon?
	\end{itemize}
...
\begin{itemize}
	...
	\begin{enumerate}
		\item[]
		\item<2->[$\rightarrow$] Wieso ist die Phrase (1) grammatisch und die Phrase (2) nicht?
		\item<2-> \textit{Die Königin von Schweden aus Deutschland} 
		\item<2->  \textit{Die Königin aus Deutschland von Schweden} 
		\item[]
		\item<3->[$\rightarrow$] Warum ist ein Satz wie (3) ungrammatisch (trotz alphabetischer Anordnung der Wörter), während (4) grammatisch ist?
		\item<3-> *\textit{Buch Chomsky das ich kaufen morgen von werde.}
		\item<3-> \textit{Das Buch von Chomsky werde ich morgen kaufen.}
		\item[]
		\item<4->[$\rightarrow$] Aus welchem Grund hat der Satz unter (5) zwei Bedeutungen? 
		\item<4-> \textit{Maria hat Peter geschlagen.}
	\end{enumerate}
...
\begin{frame}{Semantische Komponente}
	...
	\begin{itemize}
		...
		\begin{enumerate}
			\item[]
			\item<3->[$\rightarrow$] Worin besteht der Bedeutungsunterschied zwischen den Verben $\langle arbeiten \rangle$ und $\langle bearbeiten \rangle$?
			\item[]
			\item<4->[$\rightarrow$] Wieso haben die Sätze (1) und (2) nicht die gleiche Bedeutung, wenn sie aus den gleichen Wörtern bestehen?
			\item<4-> \textit{Maria hat Peter gesehen.}
			\item<4-> \textit{Hat Maria Peter gesehen?}
			\item[]
			\item<5->[$\rightarrow$] Warum bedeutet $\langle sich \rangle$ in (3) und (4) nicht dasselbe?
			\item<5-> \textit{Maria verspricht sich Mario zu treffen.}
			\item<5-> \textit{Maria verspricht Mario sich zu treffen.}
		\end{enumerate}
...
\centering
	...
	\caption{Architektur des Sprachsystems \citep{Repp&Co12a}}
	\label{Zeichen3}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item[]
			\item<2-> Arbeit mit \textbf{empirischen} Verfahren wie Experimenten (z.~B. Psycholinguistik) oder wie Ansammlungen von Daten (z.~B. Korpuslinguistik)) als Evidenz $\rightarrow$ \textbf{Naturwissenschaft}
			\item[]
			\item<3-> Beschäftigung mit der \textbf{Geschichte} einer Sprache (z.~B. Historische Linguistik) und mit den \textbf{sozialen} und kulturellen Bedingungen vom Sprachwandel (z.~B. Soziolinguistik) $\rightarrow$ \textbf{Geisteswissenschaft}
			\item[]
...
\begin{frame}{Sprachwissenschaft vs. Linguistik}
	...
	\begin{itemize}
		\item<1-> Linguistik und Sprachwissenschaft i.~d.~R. \textbf{synonymisch} gebraucht
		\item[]
